...
def load():
  ...
  QFEATURES = [ "tag", "upvotes", "answers", "top_answers" ] + qWords.columns.tolist() + qChars.columns.tolist()
  UFEATURES = uWords.columns.tolist() + uChars.columns.tolist() + uTags.columns.tolist()

  FEATURES = QFEATURES + UFEATURES
  TARGET = 'answered'

  detailed_question_info = pd.concat([question_info, qWords, qChars], axis=1, join_axes=[question_info.index])
  detailed_user_info     = pd.concat([user_info, uWords, uChars, uTags], axis=1, join_axes=[user_info.index])
...
TRAIN_DATA, TEST_DATA, FEATURES, TARGET = load()
gc.collect()
